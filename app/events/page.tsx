...
async function getEvents(): Promise<Event[]> {
  const events = await client.fetch(
    `*[_type == "event" && dateTime(date) >= dateTime(now()) - 60*60*6 && (!defined(publishAt) || dateTime(publishAt) <= dateTime(now()))] | order(date asc) {
  ...
  return (
    <main className="min-h-screen bg-gate-black text-gate-white pt-[57px]">
      ...
      <section className="max-w-6xl mx-auto px-4 py-16">
        <h1
          className="text-[clamp(2rem,7.5vw,6rem)] leading-none uppercase mb-12 text-center"
